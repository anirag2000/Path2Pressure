...
SecurityManager securityManager;
    ...
    public void updatePoint(float x, float y)
    {
       //  Enforce a limit of 1g or 9.8 m/s^2
        Log.w("gravity", Double.toString(SensorManager.GRAVITY_EARTH) );
       ...
       GaugeAcceleration.xf = scaleX* -x +  rimRect.centerX();
        GaugeAcceleration.yf =scaleY* y +  rimRect.centerY();
    ...
    private void initDrawingTools()
    {
        ...
        scaleX = ((rimRect.right - rimRect.left) / (SensorManager.GRAVITY_EARTH ));

        scaleY = ((rimRect.bottom - rimRect.top) / (SensorManager.GRAVITY_EARTH ));
        Log.w("scale", Float.toString(scaleX)+Float.toString(scaleY) );
        // inner rim oval
        ...
        rimShadowPaint.setColor(getResources().getColor(R.color.colorPrimary,getContext().getTheme()));
        ...
        pointPaint.setShadowLayer(0.01f, -0.005f, -0.005f, color);
    ...
    private void drawPoint(Canvas canvas)
    {
        ...
        canvas.drawCircle(xf,yf, 0.025f, pointPaint);
        canvas.restore();
        Log.w("UPDATING", Double.toString(GaugeAcceleration.xf)+Double.toString(GaugeAcceleration.yf) );
        invalidate();
    }
